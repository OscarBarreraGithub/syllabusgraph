...
| C16 | **Implemented:** bounded revisions and critic adjudication with decision history | `agent dispatch/complete --stage adjudicate`; [agent code](../src/syllabusgraph/agents.py), [agent tests](../tests/test_agents.py) | Two adverse reviews by default force escalation; rejection escalates immediately. Overrides need cited decisions and a fresh independent critic. Missing evidence can be deferred, not fabricated. |
