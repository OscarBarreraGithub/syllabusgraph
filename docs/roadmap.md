...
|---|---|---|
| Conceptual map | **Next:** build a consistent concept layer over extracted records, preserving source-specific treatments and distinguishing topic coverage from equivalence; start with one bounded topic before scaling. Present a compact map with detail on selection. | Reviewed mappings and a usable topic-level view; see the [abstraction audit](graph-abstraction.md). Exact-overlap counts and passing UI checks alone are insufficient. |
