...
|---|---|---|
| A graph passes browser tests but lags during use | Profile hover, pan, zoom, and focus transitions on the full inventory, with CPU slowdown and a high-density display. Inspect frame intervals and long tasks, not only JavaScript duration. | Avoid per-record DOM updates on every pointer event. Coalesce rendering, cache static backgrounds, and keep selected detail sharp. Re-measure the whole interaction: moving work from SVG to canvas can still stall during rasterization. Keep a reproducible comparison; functional checks alone do not establish responsiveness. |
| A navigation tab is inert or its purpose is unclear | Start with no selection, then select a point and try the tab. Is the current selection shared by the two views? | Give the empty view a short explanation and an actionable next step. Open the current selected record and verify reload and Back. |
...
| An accepted inventory still has an obvious wrong treatment | Open the full original record. Did a shared term hide a different meaning, such as spontaneous emission versus symmetry breaking? | Treat this as a bounded scientific amendment with new evidence and the linked family budget. Preserve the original verdict and record the correction. Do not silently change accepted mappings or reopen a whole-subject reviewer loop. |
...
| Browser zoom makes the graph cramped or controls disappear | Does the canvas retain a desktop minimum width? Is a sidebar consuming most of the resized viewport? | Reflow the map and move details below it when necessary. Test actual browser zoom, not just device scale. Keep readable labels, a fit action, and reachable controls; respect reduced motion for transitions. |
| A working view is unreachable from another tab | Do view-specific styles hide navigation? Can a horizontal tab strip leave options off-screen? | Keep available views in a consistent, wrapping navigation row. Test a round trip through every tab from both the homepage and saved URLs. Checking only the active canvas misses this failure. |
| “All records” opens an overwhelming list | Does the action show structure or only repeat record descriptions? | Open the full graph with compact points and selective labels; use a short search list to select a point. Retain chapter and pair filters in the URL and test reload/back navigation through evidence. |
| Responsive tests pass while information disappears | Are hidden sidebars excluded from the assertions? | Explicitly assert that required statistics, comparison controls, and evidence remain available. Move them below the graph and check a real screenshot; absence of horizontal overflow alone is not success. |
| The page is wider than the window but every visible control fits | Do hidden accessibility labels, text overflow, or SVG contents extend a container's scroll width? | Inspect both bounding boxes and scroll widths, and capture the failing browser's screenshot. Position visually hidden labels explicitly; preserve their accessibility. A delay or clipped graph cannot fix overflow elsewhere in the page. |
